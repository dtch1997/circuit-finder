...
import torch as t
from transformer_lens import HookedTransformer, HookedTransformerKeyValueCache, HookedSAETransformer
...
import circuit_finder.auto_circuit.model_utils.hooked_sae_transformer_utils as hooked_sae_transformer_utils
...
def graph_edges(
    ...
    if not factorized:
        ...
        if isinstance(model, MicroModel):
            ...
            assert not separate_qkv, "separate_qkv not implemented for HookedSAETransformer"
            srcs: Set[SrcNode] = hooked_sae_transformer_utils.factorized_src_nodes(model)
            dests: Set[DestNode] = hooked_sae_transformer_utils.factorized_dest_nodes(model, False)
        elif isinstance(model, HookedTransformer):
