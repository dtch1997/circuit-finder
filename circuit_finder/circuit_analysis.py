# type: ignore
# flake8: noqa
"""Code for doing circuit analysis with transcoders"""
...
def get_attn_head_contribs(model, cache, layer, range_normal):
    ...
    weighted_vals = torch.einsum(
        "b h d s, b s h f -> b h d s f", attn_pattern, split_vals
    )

    # 'batch head dst src d_head, head d_head d_model -> batch head dst src d_model'
    weighted_outs = torch.einsum(
        "b h d s f, h f m -> b h d s m", weighted_vals, model.W_O[layer]
    )
        ...
        if token_idxs is not None:
            ...
            with torch.no_grad():
                _, cache = model.run_with_cache(
                    tokens_arr[example_idx, : token_idx + 1],
            ...
            with torch.no_grad():
                _, cache = model.run_with_cache(
                    tokens_arr[i : i + batch_size],
                    stop_at_layer=layer + 1,
                    names_filter=[act_name],
                )
                acts = cache[act_name]
                feature_activs = transcoder(acts)[1].reshape(
                    -1, transcoder.W_enc.shape[1]
                )

                cur_ixg = torch.einsum(
                    "i, ji -> ji", pulledback_feature, feature_activs
                )
...
def get_ln_constant(model, cache, vector, layer, token, is_ln2=False, recip=False):
    x_act_name = (
        get_act_name("resid_mid", layer) if is_ln2 else get_act_name("resid_pre", layer)
    )
...
class Component:
    ...
    def __str__(self, show_token=True):
        ...
        attn_str = (
            "" if self.component_type != ComponentType.ATTN else f"[{self.attn_head}]"
        )
...
class FeatureVector:
    ...
    def __str__(self, show_full=True, show_contrib=True, show_last_token=True):
        retstr = ""
        token_str = (
            "" if self.token is None or not show_last_token else f"@{self.token}"
        )
        if len(self.component_path) > 0:
            if show_full:
                retstr = "".join(
                    x.__str__(show_token=False) for x in self.component_path[:-1]
                )
            retstr = "".join(
                [retstr, self.component_path[-1].__str__(show_token=False), token_str]
            )
        else:
...
def get_top_transcoder_features(model, transcoder, cache, feature_vector, layer, k=5):
    ...
    is_transcoder_post_ln = (
        "ln2" in transcoder.cfg.hook_point and "normalized" in transcoder.cfg.hook_point
    )
    ...
    error = torch.dot(feature_vector.vector, mlp_out - transcoder_out) / torch.dot(
        feature_vector.vector, mlp_out
    )
...
def get_top_contribs(
    ...
    for cur_layer in range(attn_max_layer):
        ...
        top_attn_contribs_flattened, top_attn_contrib_indices_flattened = torch.topk(
            attn_contribs.flatten(), k=np.min([k, len(attn_contribs)])
        )
        top_attn_contrib_indices = np.array(
            np.unravel_index(
                to_numpy(top_attn_contrib_indices_flattened), attn_contribs.shape
            )
        ).T
...
def greedy_get_top_paths(
    ...
    if new_root.component_path[-1].feature_type == FeatureType.TRANSCODER:
        ...
        new_root.contrib = tc(cache[tc.cfg.hook_point])[1][
            ...
            if do_cap:
                ...
                for cap_feature in path:
                    if len(cap_feature.component_path) > 0 and (
                        ...
                        or cap_feature.component_path[-1].feature_type
                        == FeatureType.SAE
        ...
        for i, cur_paths in enumerate(paths):
            ...
            for j, cur_path in enumerate(cur_paths):
                ...
                print(
                    " <- ".join(
                        map(
                            lambda x: x.__str__(show_full=False, show_last_token=True),
                            cur_path,
                        )
                    )
                )
...
@torch.no_grad()
def get_raw_top_features_among_paths(
    all_paths, use_tokens=True, top_k=5, filter_layers=None, filter_sublayers=None
):
    retdict = {}
    str_to_feature = {}
    for i, cur_length_paths in enumerate(all_paths):
        for j, cur_path in enumerate(cur_length_paths):
            ...
            if filter_layers is not None and cur_feature.layer not in filter_layers:
                continue
            if (
                filter_sublayers is not None
                and cur_feature.sublayer not in filter_sublayers
            ):
                continue

            cur_feature_str = cur_feature.__str__(
                show_contrib=False, show_last_token=use_tokens
            )
            ...
            if cur_feature_str not in retdict:
                ...
                retdict[cur_feature_str].contrib = (
                    retdict[cur_feature_str].contrib + contrib
                )
    if top_k is None or top_k > len(retdict):
        top_k = len(retdict)
    top_scores, top_indices = torch.topk(
        torch.tensor([x.contrib for x in retdict.values()], dtype=torch.float), k=top_k
    )
    retlist = []
    ...
    for score, index in zip(top_scores, top_indices):
        ...
        if not use_tokens:
            for component in cur_feature.component_path:
        ...
        for field in dataclasses.fields(self):
            ...
            if filter_level == "feature":
                if val is not None:
                    ...
                    if filter_type == FilterType.EQ and val != feature.__dict__[name]:
                        return False
                    if filter_type == FilterType.NE and val == feature.__dict__[name]:
                        return False
                    if filter_type == FilterType.GT and feature.__dict__[name] <= val:
                        return False
                    if filter_type == FilterType.GE and feature.__dict__[name] < val:
                        return False
                    if filter_type == FilterType.LT and feature.__dict__[name] >= val:
                        return False
                    if filter_type == FilterType.LE and feature.__dict__[name] > val:
                        return False
                ...
                if val is not None:
                    filter_type = self.__dict__[f"{name}_filter_type"]
                    if filter_type == FilterType.EQ and val != component.__dict__[name]:
                        return False
                    if filter_type == FilterType.NE and val == component.__dict__[name]:
                        return False
        return True
...
def path_to_str(path, show_contrib=False, show_last_token=False):
    return " <- ".join(
        list(
            x.__str__(show_contrib=show_contrib, show_last_token=show_last_token)
            for x in path
        )
    )
...
def paths_to_graph(all_paths):
    ...
    for i, cur_path in enumerate(path_list):
        for j in range(0, len(cur_path)):
            ...
            if j == 0:
                ...
                try:
                    my_contrib = (
                        prefix[0].contrib if prefix[0].contrib is not None else 0
                    )
                    nodes[prefix_str].contrib += my_contrib
                    if prefix[0].contrib is not None and my_contrib != 0:
                        nodes[prefix_str].vector = (
                            nodes[prefix_str].vector + prefix[0].vector
                        )
                except KeyError:
            ...
            parent_str = parent.__str__(
                show_contrib=False, show_last_token=True, show_full=False
            )
            child_str = child.__str__(
                show_contrib=False, show_last_token=True, show_full=False
            )
            assert (
                child.contrib_type == ContribType.RAW
            ), f"Contrib type is not ContribType.RAW: {child_str}->{parent_str}"
    ...
    for node_str in nodes:
        node = nodes[node_str]
        if node.component_path[-1].component_type != ComponentType.ATTN:
            continue
        ...
        try:
            ...
            pass
        new_nodes[
            node.__str__(show_contrib=False, show_last_token=True, show_full=False)
        ] = node
...
def add_error_nodes_to_graph(model, cache, transcoders, edges, nodes, do_bias=True):
    ...
    for child_str, parent_str in edges:
        ...
        if parent.sublayer not in ["resid_mid", "resid_pre", "resid_post"]:
            continue

        max_layer = parent.layer
        if parent.sublayer == "resid_post":
        ...
        error_str = error_feature.__str__(
            show_contrib=False, show_last_token=True, show_full=True
        )
    ...
    if do_bias:
        ...
        for parent_str in children_dict:
            # print(parent_str)
            parent = nodes[parent_str]
            if parent.component_path[-1].feature_type not in [FeatureType.TRANSCODER]:
                continue  # [FeatureType.TRANSCODER, FeatureType.SAE]
            bias = (
                -transcoders[parent.layer].W_enc[
                    :, parent.component_path[-1].feature_idx
                ]
                @ transcoders[parent.layer].b_dec
                + transcoders[parent.layer].b_enc[parent.component_path[-1].feature_idx]
            ).item()
...
def sum_over_tokens(edges, nodes):
    ...
    for (child_str, parent_str), score in edges.items():
        new_child_str = nodes[child_str].__str__(
            show_contrib=False, show_last_token=False, show_full=False
        )
        new_parent_str = nodes[parent_str].__str__(
            show_contrib=False, show_last_token=False, show_full=False
        )
...
def layer_to_float(feature):
    layer = feature.layer
    if feature.sublayer == "resid_mid":
        layer = layer + 0.5
    if feature.component_path[-1].component_type in [
        ComponentType.PRUNE_ERROR,
        ComponentType.BIAS_ERROR,
        ComponentType.TC_ERROR,
    ]:
        layer = layer - 0.25
    return layer
...
def nodes_to_coords(nodes, y_jitter=0.3, y_mult=1.0):
    ...
    for node_name in retdict:
        ...
        retdict[node_name][1] = (
            retdict[node_name][1] / (num_nodes - 1) if num_nodes != 1 else 0.5
        )
        if num_nodes != 1:
            cur_y_noise = np.random.uniform(0, y_jitter)
            if retdict[node_name][1] > 0.5:
                cur_y_noise *= -1
...
def plot_graph(
    ...
    width=800,
    height=600,
    arrow_width_multiplier=3.0,
    only_get_contribs_in_graph=False,
):
    # TODO: ugly code reuse with feature_dashboard
    def batch_color_interpolate(
        scores, max_color, zero_color, scores_min=None, scores_max=None
    ):
        if scores_min is None:
        ...
        zero_color_vec = np.array(
            [
                int(zero_color[1:3], 16),
                int(zero_color[3:5], 16),
                int(zero_color[5:7], 16),
            ]
        )

        color_vecs = np.einsum(
            "i, j -> ij", scores_normalized, max_color_vec
        ) + np.einsum("i, j -> ij", 1 - scores_normalized, zero_color_vec)
        color_strs = [
            f"#{int(x[0]):02x}{int(x[1]):02x}{int(x[2]):02x}" for x in color_vecs
        ]
        return color_strs

    arrow_widths = np.array(list(edges.values()))
    arrow_widths = 1 + arrow_width_multiplier * (arrow_widths - arrow_widths.min()) / (
        arrow_widths.max() - arrow_widths.min()
    )

    layout = nodes_to_coords(nodes, y_mult=y_mult)
    colors = batch_color_interpolate(
        np.array([x.contrib if x.contrib is not None else 0 for x in nodes.values()]),
        "#22ff22",
        "#ffffff",
    )
    ...
    trace = go.Scatter(
        ...
        hoverinfo="text",
        hovertext=[
            f"{key}<br>Contrib: {val.contrib:.2}"
            if val.contrib is not None
            else f"{key}"
    ...
    trace2 = go.Scatter(
        ...
        hoverinfo="text",
        hovertext=[
            f"{edge[0]} -> {edge[1]}<br>Contrib: {contrib:.2}"
