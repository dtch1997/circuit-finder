    ...
    for i in tqdm.tqdm(range(0, tokens_arr.shape[0], batch_size)):
        with torch.no_grad():
            ...
            if feature_post is None:
                feature_post = (
                    encoder.W_enc[:, feature_idx]
                    if not use_decoder
                    else encoder.W_dec[feature_idx]
                )
            bias = (
                -(encoder.b_dec @ feature_post)
                if use_decoder
                else encoder.b_enc[feature_idx] - (encoder.b_dec @ feature_post)
            )
            if use_raw_scores:
        ...
        scores.append(
            to_numpy(cur_scores.reshape(-1, tokens_arr.shape[1])).astype(np.float16)
        )
...
def get_uniform_band_examples(
    scores, uniform_vals, uniform_idxs, num_bands, band_size, return_percentages=False
):
    retlist = []
    ...
    for band in range(0, total_num_exs + band_size, band_size):
        ...
        num_examples_in_band = (
            np.sum(np.logical_and(scores > low_score, scores <= high_score)) / denom
        )
        retlist.append(
            (
                low_score,
                ...
                uniform_idxs[
                    np.logical_and(
                        uniform_vals >= low_score, uniform_vals <= high_score
                    )
                ],
            )
        )
...
def display_activating_examples_dash(
    model,
    all_tokens,
    scores,
    num_examples=50,
    ...
    display(
        HTML(
            f"<{header_tag} style='font-family: serif'>Firing frequency: {100*np.sum(scores > 0)/np.prod(scores.shape):.4f}%</{header_tag}>"
        )
    )
    uniform_vals, uniform_idxs = sample_uniform(scores, num_examples, unique=True)
    unif_bands = get_uniform_band_examples(
        scores,
        uniform_vals,
        uniform_idxs,
        num_bands,
    ...
    for band in reversed(unif_bands):
        cur_html_list = [
            f"<details><summary><{header_tag} style='display: inline; font-family: serif'>Between {band[0]:.2f} and {band[1]:.2f}: {100*band[2]:.4f}%</{header_tag}></summary>"
        ]
        for example_idx, token_idx in reversed(band[3]):
            cur_html_list.append(
                ...
                + f"<span> Example {example_idx}, token {token_idx}</span>"
                + "<br/>"
            )
...
"""
    ...
    top_color_strs = batch_color_interpolate(
        top_scores, "#7f7fff", "#ffffff", scores_min=scores_min, scores_max=scores_max
    )
    bot_color_strs = batch_color_interpolate(
        -bot_scores, "#ff7f7f", "#ffffff", scores_min=scores_min, scores_max=scores_max
    )
...
def plot_pulledback_feature(
    ...
    with torch.no_grad():
        ...
        if input_example is not None:
            ...
            _, cache = model.run_with_cache(
                prompt,
                stop_at_layer=input_layer + 1,
                names_filter=get_act_name(f"normalized{input_layer}ln2", input_layer),
            )
            feature_activs = transcoder(
                cache[get_act_name(f"normalized{input_layer}ln2", input_layer)]
            )[1][0, input_token_idx]
    ...
    if do_plot:
        ...
        score_min = -np.min(
            np.abs([pulledback_feature.max(), pulledback_feature.min()])
        )
        colors = batch_color_interpolate(
            pulledback_feature, "#7f7fff", "#ff7f7f"
        )  # , scores_min=score_min, scores_max=score_max)
        ...
        if type(input_example) is int:
            plt.title(
                f"Example {input_example} token {input_token_idx}:\n {str(feature_vector)} from transcoder features"
            )
        elif type(input_example) is str:
            plt.title(
                f"Connections on prompt:\n {str(feature_vector)} from transcoder features"
            )
        else:
            plt.title(
                f"Input-independent connections:\n {str(feature_vector)} from transcoder features"
            )
        plt.show()
    return pulledback_feature


def get_transcoder_pullback_features(
    model,
    feature_vector,
    transcoder,
    k=7,
    ...
    pulledback_feature = plot_pulledback_feature(
        model,
        feature_vector,
        transcoder,
        do_plot=do_plot,
...
def display_transcoder_pullback_features(
    model,
    feature_vector,
    transcoder,
    k=7,
    ...
    logits = list(
        get_transcoder_pullback_features(
            model,
            feature_vector,
            transcoder,
            k=k,
...
def get_ov_norms_for_transcoder_feature(model, transcoder, feature_idx, layer=None):
    with torch.no_grad():
        propagated_vecs = torch.einsum(
            "lhio,o->lhi", model.OV.AB, transcoder.W_enc[:, feature_idx]
        )
        ...
        ax.set_title(
            f"OV de-embedding norms for transcoder feature {feature_idx}", fontsize=10
        )
        plt.show()
...
def plot_deembedding_for_transcoder_feature(
    model, transcoder, feature_idx, attn_head=None, attn_layer=0
):
    pulledback_feature = get_deembeddings_for_transcoder_feature(
        model, transcoder, feature_idx, attn_head=attn_head, attn_layer=attn_layer, k=0
    )
...
def display_deembeddings_for_transcoder_feature(
    model, transcoder, feature_idx, attn_head=None, attn_layer=0, k=7
):
    pulledback_feature, deembeddings = get_deembeddings_for_transcoder_feature(
        model, transcoder, feature_idx, attn_head=attn_head, attn_layer=attn_layer, k=k
    )
...
def display_deembeddings_for_feature_vector(model, feature_vector, k=7):
    pulledback_feature, deembeddings = get_deembeddings_for_feature_vector(
        model, feature_vector, k=k
    )
    deembeddings = list(deembeddings)
...
def display_analysis_for_transcoder_feature(
    model, transcoder, feature_idx, attn_k=2, k=8, layer=None
):
    ...
    ov_norms = get_ov_norms_for_transcoder_feature(
        model, transcoder, feature_idx, layer=layer
    )
    top_ov_norms, top_ov_heads_flattened = torch.topk(ov_norms.flatten(), k=attn_k)
    top_ov_indices = np.array(
        np.unravel_index(to_numpy(top_ov_heads_flattened), ov_norms.shape)
    ).T

    colors = batch_color_interpolate(
        to_numpy(top_ov_norms),
        "#ff7f7f",
        "#ffffff",
        scores_min=0,
        scores_max=top_ov_norms.max().item(),
    )
    ...
    for cur_layer, head in top_ov_indices:
        display_deembeddings_for_transcoder_feature(
            model, transcoder, feature_idx, attn_head=head, attn_layer=cur_layer
        )
        plot_deembedding_for_transcoder_feature(
            model, transcoder, feature_idx, attn_head=head, attn_layer=cur_layer
        )
