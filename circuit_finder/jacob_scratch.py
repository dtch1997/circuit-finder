...
def load_attn_saes() -> dict[LayerIndex, tl.HookedSAE]:
    ...
    for module_name, sae in sae_dict.items():
        ...
        W_dec_z = rearrange(sae.W_dec, "nf (nhead dhead) -> nf nhead dhead", nhead=model.cfg.n_heads)
...
class CircuitFinder:
    def __init__(
        ...
        corrupt_tokens : Int[torch.Tensor, "batch seq"] = None # only specify if contrast_pairs = True
    ...
    def get_initial_cache(self):
        ...
        if self.cfg.contrast_pairs:
            names_filter = lambda x: (x.endswith("ln2.hook_normalized") or x.endswith("mlp_out") or x.endswith("hook_z"))
            _, corrupt_cache = self.model.run_with_cache(self.corrupt_tokens,
                                        return_type="loss",
                                        names_filter = names_filter)
        ...
        for layer in range(self.n_layers):
            ...
            attn_recons, sae_cache = self.attn_saes[layer].run_with_cache(z_concat, names_filter='hook_sae_acts_post')
            attn_feature_acts = sae_cache['hook_sae_acts_post']

            if self.cfg.contrast_pairs:
                z_concat = rearrange(corrupt_cache[attn_out_pt], "batch seq n_heads d_head -> batch seq (n_heads d_head)")
                attn_recons, sae_cache = self.attn_saes[layer].run_with_cache(z_concat, names_filter='hook_sae_acts_post')
                attn_feature_acts -= sae_cache['hook_sae_acts_post']
            ...
            resid_error = einsum(z_error, self.model.W_O[layer], "seq n_heads d_heads, n_heads d_head d_model -> seq d_model")
