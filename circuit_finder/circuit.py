...
class CircuitFinder:
    ...
    def get_initial_cache(self):
        ...
        self.headmean_pattern = torch.stack(
            [cache["pattern", layer].mean(dim=[0, 1]) for layer in self.layers]
        )  # [layer qpos kpos]
    ...
    def ov_step(self, down_layer):
        ...
        imp_layernorm_scales = self.attn_layernorm_scales[
            down_layer, imp_down_pos
        ].unsqueeze(0)  # [1, imp_id, 1]

        # ..
        W_V_concat = rearrange(
            self.model.W_V[down_layer],
            "head_id d_model d_head -> d_model (head_id d_head)",
        )
        grad = einsum(
            imp_active,
            imp_enc_rows,
    ...
    def get_active_attn_W_dec(self, down_layer):
        ...
        attn_up_active_layers = self.attn_active_feature_ids[0][
            self.attn_active_feature_ids[0] < down_layer
        ]
    ...
    def compute_and_save_attribs(
        ...
        attn_active_W_dec, attn_up_active_layers, attn_up_active_feature_ids = (
            self.get_active_attn_W_dec(down_layer)
        )
