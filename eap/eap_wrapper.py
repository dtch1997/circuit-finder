...
def EAP_corrupted_forward_hook(
    ...
    if activations.ndim == 3:
        ...
        upstream_activations_difference[:, :, hook_slice, :] = -activations.unsqueeze(
            -2
        )
...
def EAP_clean_backward_hook(
    ...
    result = (
        ...
        .sum(dim=0)
        .sum(dim=0)
    )  # we sum over the batch_size and seq_len dimensions
...
def EAP_downstream_patching_hook(
    ...
    if activations.ndim == 3:
        assert (
            patch_difference.shape[-2] == 1
        ), "Number of downstream nodes should be 1 for this type of hook"
...
def EAP(
    ...
    ), "Shape mismatch between clean and corrupted tokens"
    num_prompts, seq_len = clean_tokens.shape[0], clean_tokens.shape[1]

    assert (
        num_prompts % batch_size == 0
    ), "Number of prompts must be divisible by batch size"
    ...
    for idx in tqdm(range(0, num_prompts, batch_size)):
        ...
        value = metric(
            model(clean_tokens[idx : idx + batch_size], return_type="logits")
        )
        value.backward()
