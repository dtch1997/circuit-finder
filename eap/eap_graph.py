...
class EAPGraph:
    def __init__(self, cfg, upstream_nodes=None, downstream_nodes=None, edges=None):
        ...
        self.element_size = torch.empty(
            (0), device=self.cfg.device, dtype=self.cfg.dtype
        ).element_size()
    ...
    def setup_graph_from_nodes(self, upstream_nodes=None, downstream_nodes=None):
        ...
        for hook_name in self.downstream_hooks:
            ...
            if (
                hook_type == "hook_q_input"
                or hook_type == "hook_k_input"
                or hook_type == "hook_v_input"
            ):
                letter = hook_type.split("_")[1].lower()
                for head_idx in range(self.cfg.n_heads):
                    self.downstream_nodes.append(f"head.{layer}.{head_idx}.{letter}")
                    self.downstream_node_index[f"head.{layer}.{head_idx}.{letter}"] = (
        ...
        activations_tensor_in_gb = (
            self.n_upstream_nodes * self.cfg.d_model * self.element_size / 2**30
        )
        print(
            f"Saving activations requires {activations_tensor_in_gb:.4f} GB of memory per token"
        )
    ...
    def get_hooks_from_nodes(self, upstream_nodes, downstream_nodes):
        ...
        for node in downstream_nodes:
            ...
            assert (
                node_type in self.valid_downstream_node_types
            ), "Invalid downstream node"
        ...
        for node in downstream_nodes:
            ...
            if not node_is_layer_specific:
                ...
                if node.startswith("resid_post") or node.startswith("mlp"):
                    ...
                    all_heads = (
                        len(node.split(".")) <= 2
                    )  # head.10 means taking all heads at layer 10
                    head_idx = (
                        None if all_heads else int(node.split(".")[2])
                    )  # we don't use this variable because we have to add the same hook whether we want one head or all
                    letters = ["q", "k", "v"]
                    if len(node.split(".")) == 4:
    ...
    def reset_scores(self):
        self.eap_scores = torch.zeros(
            (self.n_upstream_nodes, self.n_downstream_nodes), device=self.cfg.device
        )
    ...
    def show(self, threshold=None, abs_scores=True, fname: str = "eap_graph.png"):
        ...
        layer_to_subgraph = {}
        layer_to_subgraph[-1] = g.add_subgraph(
            name="cluster_-1", rank="same", color="invis"
        )
        layer_to_subgraph[-1].add_node("-1_invis", style="invis")
        ...
        for layer in layers:
            layer_to_subgraph[layer] = g.add_subgraph(
                name=f"cluster_{layer}", rank="same", color="invis"
            )
            layer_to_subgraph[layer].add_node(f"{layer}_invis", style="invis")

            if prev_layer is not None:
                g.add_edge(
                    f"{prev_layer}_invis", f"{layer}_invis", style="invis", weight=1000
                )
        ...
        for edge in edges:
            ...
            child_name = (
                child_name.replace(".q", "").replace(".k", "").replace(".v", "")
            )
