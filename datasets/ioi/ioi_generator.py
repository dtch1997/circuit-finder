    ...
    for idx, (prompt, template) in enumerate(zip(prompts, templates_by_prompt)):
        ...
        rand_names = {
            ...
            letter: np.random.choice(
                list(sorted(set(names) - set(orig_names), key=names.index))
            )
            for letter in list(
                sorted(set(flip_new) - set(flip_orig), key=flip_new.index)
            )
        }
...
def get_word_idxs(prompts, word_list, tokenizer):
    ...
    for prompt in prompts:
        ...
        if idx is None:
            raise ValueError(f"Word {word_list} and {i} not found {prompt}")  # type: ignore
        idxs.append(idx)
...
for order in ["ABBA", "BABA"]:
    ...
    for template_idx in range(15):
        ...
        word_idxs = dict(
            [
                (word, idx[0].item() + (0 if word == "starts" else 0))
                for word, idx in ioi_dataset.word_idx.items()
            ]
        )
        ...
        path = repo_path_to_abs_path(
            f"datasets/ioi/ioi_{order}_template_{template_idx}_prompts.json"
        )
